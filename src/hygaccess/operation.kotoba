(ns hygaccess.operation
  "HygieneAccessOperationActor -- one coordination request = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (HygieneAccessAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Hygiene Access
  ...
  -> decide -> commit | hold | request-approval). No unbounded inner
  loop -- each operation is auditable and checkpointed. This actor's
  lifecycle is advanced by MANY ops (formulation-batch logging /
  maintenance scheduling / safety-concern flagging / shipment
  coordination / packaging-design proposal / market-entry proposal /
  marketing-claim proposal), each independent.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human go-to-market coordinator. The approver resumes
...
(defn build
  "Compiles a HygieneAccessOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `hygaccess.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; HygieneAccessAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Hygiene Access Operations Governor -- independent censor.
      (g/add-node :govern
