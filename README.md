...
- **Is NOT a real Manufacturing Execution System (MES) connection or real equipment control** — `hygaccess.mes` is an INTERFACE CONTRACT (`dcs.ports/IFieldIO`, `kotoba-lang/dcs`) plus a mock implementation, `mock-plant-io`, that is still a fixed, documented, non-real reference implementation (its mixing-homogeneity reading is genuinely CFD-COMPUTED, see ADR-0004, but that CFD solve is pure math with no physical actuation of anything). No code in this repo connects to, polls, or controls real plant equipment.
...
5. **`hygaccess.mes`** (MES integration CONTRACT + mock, `docs/adr/0003-mes-regulatory-sales-extensions.md`, engine rewired onto `dcs.ports/IFieldIO` + a real CFD solve by `docs/adr/0004-shared-infrastructure-rewire.md`): the `dcs.model` mixing-tank tag registry, the `mock-plant-io` reference implementation (NOT a real plant connection), and `real-cfd-homogeneity-cov-pct`, which genuinely invokes `kotoba-lang/kami-app-hygaccess-plant`'s own tested CFD solve (memoized)
