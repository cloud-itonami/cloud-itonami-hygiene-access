...
- A Manufacturing Execution System (MES) integration CONTRACT layer (`hygaccess.mes`) — what a REAL plant's MES would need to expose (batch-telemetry reading), backed by `kotoba-lang/dcs`'s host-injected `IFieldIO` tag-registry seam (`docs/adr/0004-shared-infrastructure-rewire.md`), whose mixing-homogeneity tag reads through the sibling repo `kotoba-lang/kami-app-hygaccess-plant`'s own REAL, tested finite-volume CFD mixing-tank solve (a genuine `deps.edn` dependency as of ADR-0004, not merely a name-only doc-comment reference) — the concrete realization of the "loose EDN-map coupling" this actor's docs have promised since that sibling repo was first named. As of `docs/adr/0005-closed-loop-control-simulation.md`, this is no longer one memoized monolithic CFD call: `hygaccess.mes/run-homogeneity-control-loop` is **a real, computed, closed-loop control simulation** — sensor read (`kami-app-hygaccess-plant.mixing/sensor-reading`) → PID compute (`kotoba-lang/dcs`'s real `dcs.pid`) → command → CFD state update (`kami-app-hygaccess-plant.mixing/step`, Repo 1's stepwise API), iterated, with a `dcs.alarm` ISA-18.2 alarm/interlock layer able to override the PID into a documented safe agitation state — **entirely within this software's own digital twin. See "The four hard boundaries, restated" below: there is no physical actuation pathway anywhere in this codebase, and none is intended.** As of `docs/adr/0006-modbus-observability-demo.md`, that same closed-loop trace is also externally OBSERVABLE (never steerable) over a real, loopback-only, read-only Modbus TCP interface (`hygaccess.modbus-demo`, built on `kotoba-lang/dcs`'s `dcs.modbus`) — a demonstration/integration-test capability, not a governed op, proving the digital twin is wire-protocol-compatible with real SCADA/HMI tooling while remaining entirely simulated.
...
- `:record-mes-reading` — logs an equipment/batch telemetry reading (temperature, pH, mixing RPM, mixing-homogeneity CoV) read through a `dcs.ports/IFieldIO` mock (`hygaccess.mes/mock-plant-io`, ADR-0004) tied to an existing batch — administrative logging, auto-eligible like `:log-production-batch`
...
- **Is NOT a real Manufacturing Execution System (MES) connection or real equipment control** — `hygaccess.mes` is an INTERFACE CONTRACT (`dcs.ports/IFieldIO`, `kotoba-lang/dcs`) plus a mock implementation, `mock-plant-io`, that is still a fixed, documented, non-real reference implementation. No code in this repo connects to, polls, or controls real plant equipment.
- **Is a real, computed, closed-loop control SIMULATION for mixing-homogeneity, with NO physical actuation pathway anywhere** (`docs/adr/0005-closed-loop-control-simulation.md`) — `hygaccess.mes/run-homogeneity-control-loop` genuinely does: sensor read (`kami-app-hygaccess-plant.mixing/sensor-reading`) → PID compute (`kotoba-lang/dcs`'s real `dcs.pid`) → command → CFD state update (`kami-app-hygaccess-plant.mixing/step`), iterated, with a `dcs.alarm` ISA-18.2 alarm/interlock able to override the PID into a documented safe agitation state. Every number is genuinely computed (real PID math over a real, iteratively-advancing finite-volume CFD state) — but the WHOLE loop runs entirely inside this software's own digital twin. No GPIO, no serial, no OPC-UA, no PLC client — none of that exists anywhere in this codebase or its dependencies, and none is intended. **One real, loopback-only, read-only Modbus TCP interface DOES exist** (`docs/adr/0006-modbus-observability-demo.md`, `hygaccess.modbus-demo`) — a demonstration/integration-test capability that lets a real Modbus client WATCH this same digital twin's live values over a real wire protocol; it has no write register and no path to steer the control loop, and it never binds beyond `127.0.0.1`.
...
### The four hard boundaries, restated (docs/adr/0003-mes-regulatory-sales-extensions.md, docs/adr/0005-closed-loop-control-simulation.md, docs/adr/0006-modbus-observability-demo.md)

This build was extended under an explicit, hard operator-safety constraint that literal chemical manufacturing, literal regulatory-agency filing, literal real-world payment execution, and literal physical equipment actuation are outside what this actor may ever do — not a style preference, a hard line:
...
4. **No physical actuation pathway anywhere, even where the control math is genuinely real, and even where a real wire protocol is genuinely spoken** (`docs/adr/0005-closed-loop-control-simulation.md`, updated by `docs/adr/0006-modbus-observability-demo.md`). `hygaccess.mes/run-homogeneity-control-loop` is a REAL closed-loop control simulation — a real PID algorithm (`kotoba-lang/dcs`'s `dcs.pid`) computing real numbers from a real, iteratively-advancing finite-volume CFD state (`kami-app-hygaccess-plant.mixing/step`), with a real ISA-18.2 alarm/interlock (`dcs.alarm`) able to override it into a safe state — but every input, output, and side effect of that loop stays inside this software's own digital twin. No GPIO, no serial, no OPC-UA, no PLC client, no equipment-control code of any kind, anywhere in this codebase or its dependencies. **One real, loopback-only, read-only Modbus TCP interface DOES now exist** (`hygaccess.modbus-demo`, `docs/adr/0006-modbus-observability-demo.md`, built on `kotoba-lang/dcs`'s `dcs.modbus`) — real MBAP wire framing, a real independent Modbus client can connect and read it — but every register it serves is read-only (function code 04 only, no write register anywhere in the map), it binds `127.0.0.1` only and refuses anything else, and it is a window ONTO this software's own digital twin, never a path INTO it: the control loop's own commanded RPM is still computed and applied entirely internally, never through this or any other external seam.
...
2. **`hygaccess.governor`** (independent, `Hygiene Access Operations Governor`): validates against domain rules, re-derived from `hygaccess.registry`'s pure functions, `hygaccess.mes`/`hygaccess.regulatory`'s domain logic, and `hygaccess.store`'s SSoT — never trusts the advisor's own self-report
   ...
   - ESCALATE (always human sign-off, overridable by a human):
     ...
     - An MES/CFD reading (or a batch's own IPQC record) whose `:control-loop-alarm-triggered?` is true — NEW, `docs/adr/0005-closed-loop-control-simulation.md`, check 40, independently re-derived (never merely self-reported `:stake`) — a batch/reading whose real closed-loop control trace hit an alarm never silently looks identical to one that converged cleanly
...
5. **`hygaccess.mes`** (MES integration CONTRACT + mock, `docs/adr/0003-mes-regulatory-sales-extensions.md`, engine rewired onto `dcs.ports/IFieldIO` + a real CFD solve by `docs/adr/0004-shared-infrastructure-rewire.md`, and onto a REAL CLOSED CONTROL LOOP by `docs/adr/0005-closed-loop-control-simulation.md`): the `dcs.model` mixing-tank tag registry (now also six ISA-18.2 alarms + one PID loop), the `mock-plant-io` reference implementation (NOT a real plant connection), `run-homogeneity-control-loop` (the real sensor-read → PID-compute → command → CFD-state-update loop, `dcs.pid` + `dcs.alarm` + `kami-app-hygaccess-plant.mixing`'s stepwise API, bounded, digital-twin-only, and — `docs/adr/0006-modbus-observability-demo.md` — an optional `:on-iteration` observation hook), and `real-cfd-homogeneity-cov-pct`/`control-loop-alarm-triggered?`, which read that loop's own memoized final trace (never a monolithic solve's final tick, never a hand-picked stub)
6. **`hygaccess.modbus-demo`** (`docs/adr/0006-modbus-observability-demo.md`, JVM-only `.clj`, NOT part of the governed op graph) — a real, loopback-only, read-only Modbus TCP server (`kotoba-lang/dcs`'s `dcs.modbus`) exposing `run-homogeneity-control-loop`'s live per-tick state for external observability; see "The four hard boundaries, restated" above
7. **`hygaccess.regulatory`** (regulatory-submission-status STATE MACHINE, `docs/adr/0003-mes-regulatory-sales-extensions.md`, engine rewired onto the shared `cloud-itonami/cloud-itonami-regulatory-tracker` library by `docs/adr/0004-shared-infrastructure-rewire.md`): the closed transition table, human-evidence-completeness check, and the non-breaking market-approval-without-submission WARNING
...
```bash
# Run tests (top-level deps.edn already pins langgraph+langchain+
# cloud-itonami-regulatory-tracker+dcs+kami-app-hygaccess-plant+crm
# local/root -- see docs/adr/0004-shared-infrastructure-rewire.md for
# the sibling-checkout layout a standalone fork needs). Includes
...
`:implemented` — `governor.cljc`/`store.cljc`/`advisor.cljc`/`registry.cljc`/`phase.cljc`/`operation.cljc`/`mes.cljc`/`regulatory.cljc` + `deps.edn` complete the governed module set, plus one additive, non-governed `.clj` namespace (`modbus_demo.clj`); tests green (183 tests / 663 assertions, up from 181/631), demo runnable, langgraph-clj integration verified. See `docs/adr/0001-architecture.md` for the original architecture, `docs/adr/0002-gmp-and-regulatory-dossiers.md` for the GMP batch-record/QA model + regulatory dossier drafts, `docs/adr/0003-mes-regulatory-sales-extensions.md` for the MES integration contract, regulatory-submission-status tracking, and sales quote/order/fulfillment workflow added on top of both, `docs/adr/0004-shared-infrastructure-rewire.md` for the REFACTOR pass onto shared/real infrastructure (`cloud-itonami-regulatory-tracker`, `dcs`, a real CFD solve, `crm.pipeline`, and a real opt-in `murakumo-main`-backed `llm-advisor`) that preserves every op/check name/semantic from the ADRs above, `docs/adr/0005-closed-loop-control-simulation.md` for the real closed-loop control simulation (PID + ISA-18.2 alarm + real stepwise CFD) that replaces the mixing-homogeneity reading's memoized monolithic CFD call — entirely within this software's own digital twin, no physical actuation pathway anywhere — and `docs/adr/0006-modbus-observability-demo.md` for the real, loopback-only, read-only Modbus TCP interface that makes that same closed-loop trace externally observable (never steerable), proving wire-protocol compatibility with real SCADA/HMI tooling while the whole boundary above stays true.
