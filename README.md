...
- **Is a real, computed, closed-loop control SIMULATION for mixing-homogeneity, with NO physical actuation pathway anywhere** (`docs/adr/0005-closed-loop-control-simulation.md`) — `hygaccess.mes/run-homogeneity-control-loop` genuinely does: sensor read (`kami-app-hygaccess-plant.mixing/sensor-reading`) → PID compute (`kotoba-lang/dcs`'s real `dcs.pid`) → command → CFD state update (`kami-app-hygaccess-plant.mixing/step`), iterated, with a `dcs.alarm` ISA-18.2 alarm/interlock able to override the PID into a documented safe agitation state. Every number is genuinely computed (real PID math over a real, iteratively-advancing finite-volume CFD state) — but the WHOLE loop runs entirely inside this software's own digital twin. No GPIO, no serial, no Modbus, no OPC-UA, no PLC client — none of that exists anywhere in this codebase or its dependencies, and none is intended.
...
### The four hard boundaries, restated (docs/adr/0003-mes-regulatory-sales-extensions.md, docs/adr/0005-closed-loop-control-simulation.md)
...
# the sibling-checkout layout a standalone fork needs)
clojure -M:test
